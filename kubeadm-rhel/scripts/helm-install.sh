...
retries "helm upgrade --install cilium cilium/cilium --set ipam.operator.clusterPoolIPv4PodCIDRList='10.244.0.0/16' -n kube-system --atomic --timeout 10m" "Failed to install Cilium helm chart."
retries "helm upgrade --install metallb metallb/metallb -n metallb-system --create-namespace --atomic" "Installing MetalLB helm chart."
retries "helm upgrade --install ingress-nginx ingress-nginx/ingress-nginx -n ingress-nginx --create-namespace" "Failed to install Ingress-Nginx helm chart."
retries "helm upgrade --install metrics-server metrics-server/metrics-server --set replicas=3  --set args[0]='--kubelet-insecure-tls' -n kube-system" "Failed to install Metrics-Server helm chart."
retries "helm upgrade --install prom-stack prometheus-community/kube-prometheus-stack -n monitoring --create-namespace" "Failed to install Kube-Prometheus-Stack helm chart."
retries "helm upgrade --install argo-cd argo/argo-cd -n argocd --create-namespace" "Failed to install ArgoCD helm chart."
retries "helm upgrade --install sealed-secrets -n kube-system --set-string fullnameOverride=sealed-secrets-controller sealed-secrets/sealed-secrets" "Failed to install Sealed-Secrets helm chart."
